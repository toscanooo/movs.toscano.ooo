...
class MoviesController: UIViewController {
    ...
    func parse(json: Data){
        ...
        if let jsonMovies = try? decoder.decode(Movies.self, from: json) {
            ...
            data.remove(at: 0)
            for movie in movies {
                data.append(CustomData(title: "\(movie.title!)", image: #imageLiteral(resourceName: "Icon-60@3x"), description: "\(movie.overview!)", url: "https://image.tmdb.org/t/p/w220_and_h330_face\(movie.poster_path!)"))
